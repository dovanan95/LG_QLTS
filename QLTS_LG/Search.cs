                    ...
                    case 2:
                        {
                            ...
                            DataAdapter = new SqlDataAdapter(
                                "SELECT Ngoai_Kho.Ma_TS, Tai_san.Ten_TS, Loai_TS_cap1.Ten_loai, Loai_TS_cap2.Ten_loai, Ngoai_Kho.ID_User, [dbo].[User].[Name],[dbo].[User].[Phone], Ngoai_Kho.Latest_Day_Out " +
                                "FROM Ngoai_Kho " +
                                "INNER JOIN Tai_san ON Ngoai_Kho.Ma_TS = Tai_san.Ma_TS " +
                                "INNER JOIN [dbo].[User] ON [dbo].[User].[ID] = [dbo].[Ngoai_Kho].[ID_User] " +
                                "INNER JOIN Loai_TS_cap1 ON Tai_san.Ma_Loai_TS_cap1 = Loai_TS_cap1.Ma_loai " +
